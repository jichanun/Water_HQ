...
const struct test_s test = {
    ...
    .max_g_offset   = 1.0f,   //500 mg for Accel Criteria C
...
static int get_st_6500_biases(long *gyro, long *accel, unsigned char hw_test, int debug)
{
    ...
    DelayXms(test.wait_ms);  //wait 200ms for sensors to stabilize
...
//    	log_i("Starting Bias Loop Reads\n");
    ...
    while (s < test.packet_thresh) {
    	DelayXms(test.sample_wait_ms); //wait 10ms to fill FIFO
		if (i2c_read(st.hw->addr, st.reg->fifo_count_h, 2, data))
			return -1;
		fifo_count = (data[0] << 8) | data[1];
		packet_count = fifo_count / MAX_PACKET_LENGTH;
